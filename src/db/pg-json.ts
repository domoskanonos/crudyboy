...
export class PostgresqlClientJSON extends DbClient {
  ...
  async search(collection: string, req: Request): Promise<any[]> {
    ...
    const sql = `SELECT id, data FROM ${collection}${whereQuery}${limitQuery}${offsetQuery}`;
    console.log(sql);
  ...
  async update(
    ...
    this.client?.query;
    const result = await this.getClient().query(
      `UPDATE ${collectionName} SET data = $1::json WHERE id = $2;`,
      [item, item.id]
    );
    return true;
  }
